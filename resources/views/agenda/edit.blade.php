                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('agenda.update', $agenda) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <!-- Nomor Agenda (Read-only) -->
                            <div>
                                <x-input-label for="nomor_agenda" :value="__('Nomor Agenda')" />
                                <x-text-input id="nomor_agenda" class="block mt-1 w-full bg-gray-100" type="text"
                                    value="{{ $agenda->nomor_agenda }}" disabled />
                            </div>
                            ...
                            <div>
                                ...
                                <x-text-input id="tanggal_masuk" class="block mt-1 w-full" type="date"
                                    name="tanggal_masuk" :value="old('tanggal_masuk', $agenda->tanggal_masuk->format('Y-m-d'))" required />
                            ...
                            <div>
                                ...
                                <select id="satker_id" name="satker_id"
                                    ...
                                    @foreach($satkers as $satker)
                                        <option value="{{ $satker->id }}" {{ old('satker_id', $agenda->satker_id) == $satker->id ? 'selected' : '' }}>{{ $satker->kode_satker }}
                            ...
                            <div>
                                ...
                                <select id="jenis_pengelolaan_id" name="jenis_pengelolaan_id"
                                    ...
                                    @foreach($jenisPengelolaans as $jenis)
                                        <option value="{{ $jenis->id }}" data-target="{{ $jenis->target_hari }}" {{ old('jenis_pengelolaan_id', $agenda->jenis_pengelolaan_id) == $jenis->id ? 'selected' : '' }}>{{ $jenis->nama_jenis }} ({{ $jenis->target_hari }} Hari)
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <x-text-input id="tanggal_target_display" class="block mt-1 w-full bg-gray-100"
                                    type="text" value="{{ $agenda->tanggal_target->format('d/m/Y') }}" disabled />
                            ...
                            <div>
                                ...
                                <select id="pic_id" name="pic_id"
                                    ...
                                    @foreach($pics as $pic)
                                        <option value="{{ $pic->id }}" {{ old('pic_id', $agenda->pic_id) == $pic->id ? 'selected' : '' }}>{{ $pic->name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mt-6">
                            ...
                            <textarea id="notes" name="notes"
                                class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full"
                                rows="3">{{ old('notes', $agenda->notes) }}</textarea>
                            <x-input-error :messages="$errors->get('notes')" class="mt-2" />
                        </div>

                        <!-- File Upload -->
                        <div class="mt-6">
                            <x-input-label for="file_uploads" :value="__('Upload Dokumen Tambahan (Max 5 file, @10MB)')" />
                            ...
                            <p class="text-xs text-gray-500 mt-1">File baru akan ditambahkan ke daftar file yang sudah
                                ada.</p>
                                    ...
                                    @foreach($agenda->file_uploads as $file)
                                        <li>
                                            <a href="{{ Storage::url($file['path']) }}" target="_blank"
                                                class="text-blue-600 hover:underline">{{ $file['original_name'] }}</a>
                                        </li>
                                    @endforeach
                                </ul>
                            </div>
